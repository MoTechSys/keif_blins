...
abstract class _Span {
  ...
  /// Horizontal extent used for RTL positioning (advance-based).
  double get rtlExtent => left + width;
...
class _Word extends _Span {
  ...
  // Keif Aldiafa patch: position RTL words by their advance width instead of
  // their ink bounds. Glyphs with negative left bearing (ر / ز in Tajawal)
  // otherwise swallow the following space.
  @override
  double get rtlExtent => metrics.advanceWidth;
...
class _Line {
  ...
  void realign(double totalWidth) {
      ...
      case TextAlign.justify:
        ...
        for (final span in spans) {
          span.offset = PdfPoint(
            isRTL
                ? delta - x - (span.offset.x + span.rtlExtent)
                : span.offset.x + x,
    ...
    if (isRTL) {
      for (final span in spans) {
        span.offset = PdfPoint(
          delta - (span.offset.x + span.rtlExtent),
          span.offset.y - baseline,
        );
      }
      return;
